$( document ).ready(function() {
  ...
  // автовысота teatarea с [data-autoresize]
  function addAutoResize() {
    document.querySelectorAll('[data-autoresize]').forEach(function (element) {
      var offset = element.offsetHeight - element.clientHeight;
      var textareaAUtiHeight = function (event) {
        event.target.style.height = 'auto';
        event.target.style.height = event.target.scrollHeight + offset + 'px';
      };
      element.addEventListener('input', textareaAUtiHeight, false);
      element.addEventListener('cut', textareaAUtiHeight, false);
      element.addEventListener('paste', textareaAUtiHeight, false);
      element.removeAttribute('data-autoresize');
    });
  }
  addAutoResize();

  // форум, форма ввода: показать кнопки по фокусу в поле
  var forumCommentTextarea = document.querySelector('.layout-forum__add-comment-text');
  forumCommentTextarea && forumCommentTextarea.addEventListener('focus', function() {
